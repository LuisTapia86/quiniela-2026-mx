...
def entry_financials(approved_count: int, config: Mapping[str, Any]) -> dict[str, int]:
    ...
    entry_fee = int(config.get("ENTRY_FEE_MXN", 200))
    admin_pct = int(config.get("ADMIN_FEE_PERCENT", 0))
    p1 = int(config.get("PRIZE_TOP1_PERCENT", 70))
    p2 = int(config.get("PRIZE_TOP2_PERCENT", 20))
    p3 = int(config.get("PRIZE_TOP3_PERCENT", 10))
